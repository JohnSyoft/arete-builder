...
export function Video({
  ...
  const renderVideoContent = () => {
    if (!src) {
      return (
        ...
        >
          ...
          <p className="text-gray-500 text-sm text-center">Click to add video</p>
          <p className="text-gray-400 text-xs text-center mt-1">Supports MP4, YouTube, Vimeo</p>
        </div>
      )
    }
    ...
    return (
      <video
        ...
        loop={loop}
        muted={muted}
        className={`${width} ${height} ${borderRadius} object-cover`}
  ...
  return (
    <div 
      ...
      className={`relative group ${margin} ${padding} ${selected ? "ring-2 ring-blue-500" : ""} ${hovered ? "ring-1 ring-blue-300" : ""}`}
    >
      {renderVideoContent()}
