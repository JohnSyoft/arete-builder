...
export function Alert({
  ...
  const getVariantColors = (alertVariant: string) => {
    switch (alertVariant) {
      case 'info':
        return { bg: '#dbeafe', text: '#1e40af', border: '#3b82f6', icon: '#3b82f6' }
      case 'success':
        return { bg: '#dcfce7', text: '#166534', border: '#22c55e', icon: '#22c55e' }
      case 'warning':
        return { bg: '#fef3c7', text: '#92400e', border: '#f59e0b', icon: '#f59e0b' }
      case 'error':
        return { bg: '#fee2e2', text: '#991b1b', border: '#ef4444', icon: '#ef4444' }
      default:
        return { bg: backgroundColor, text: textColor, border: borderColor, icon: textColor }
    }
  }
        ...
        {showIcon && (
          <div className="flex-shrink-0 mt-0.5">
            <IconComponent 
              ...
              style={{ color: variant === 'default' ? textColor : variantColors.icon }}
            />
          </div>
        )}
